          ...
          {filteredMatches.map(match => {
            ...
            return (
              ...
              >
                ...
                {/* Collapsible Predictions of other employees */}
                <div className="mt-4 pt-4 border-t border-dashed border-slate-200">
                  <button
                    onClick={() => togglePredictions(match.id)}
                    className="w-full flex items-center justify-between text-xs font-bold text-blue-700 hover:text-blue-900 focus:outline-none transition-all cursor-pointer bg-slate-50 hover:bg-blue-50/40 py-2.5 px-3 rounded-xl border border-slate-150"
                  >
                    <span className="flex items-center gap-1.5 text-blue-800">
                      📊 {expandedPredictions[match.id] ? 'Ocultar' : 'Ver'} pronósticos de empleados ({
                        allForecasts.filter(f => f.matchId === match.id).length
                      })
                    </span>
                  ...
                  {expandedPredictions[match.id] && (
                    <div className="mt-3 space-y-2 max-h-48 overflow-y-auto pr-1">
                      {allForecasts.filter(f => f.matchId === match.id).length > 0 ? (
                        allForecasts
                          .filter(f => f.matchId === match.id)
                          .map(f => {
                            const isExact = f.pointsEarned === 3;
                            const isWin = f.pointsEarned === 1;
                            const isUser = f.userId === userId;

                            return (
                              <div
                                key={f.id}
                                className={`flex items-center justify-between text-xs p-2.5 rounded-lg border leading-tight transition-all ${
                                  isUser 
                              ...
                              >
                                <div className="flex items-center gap-2 max-w-[65%]">
                                  <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                                    isExact ? 'bg-yellow-405 bg-yellow-400' : isWin ? 'bg-blue-600' : 'bg-slate-300'
                                  }`}></span>
                                  <div className="flex flex-col truncate">
                                    <span className={`font-semibold truncate ${isUser ? 'text-blue-900 font-bold' : 'text-slate-800'}`}>
                                      {f.userName} {isUser && '(Tú)'}
                                    </span>
                                    <span className="text-[9px] text-slate-400 truncate font-mono">{f.userEmail}</span>
                                  </div>
                                </div>

                                <div className="flex items-center gap-2 shrink-0">
                                  <span className="font-extrabold font-mono text-slate-800 bg-white border border-slate-100 px-2 py-0.5 rounded shadow-sm text-xs">
                                  ...
                                  {match.status === 'finished' && (
                                    <span className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded font-mono ${
                                      isExact 
                                        ...
                                        : isWin 
                                          ? 'bg-blue-105 bg-blue-100 text-blue-900 border border-blue-200'
                                          : 'bg-slate-100 text-slate-500 border border-slate-205 border-slate-200'
                                    }`}>
                                      +{f.pointsEarned ?? 0}
                                    </span>
                                  )}
                        ...
                        <p className="text-[11px] text-slate-400 text-center py-3 italic font-medium">
                          Nadie ha cargado pronósticos aún para este partido
                        </p>
                      )}
                    </div>
                  )}
